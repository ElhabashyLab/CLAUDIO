...
def download_pdbs(dataset:pd.DataFrame, search_tool: str, res_cutoff: float,
                  output_directory: str, verbose_level: int):
    """
    Download pdb files either from RCSB or AlphaFold database (depending on 
    earlier hhsearch or blastp search) into output directory
...
def clear_output_dir(search_tool: str, output_directory: str):
    """
    Clear output directory of pdb files starting with the specified search 
    tool's name, to ensure that no old results or other files intervene in
    later parsing of the results
...
def download_pdb_from_db(url: str, i_try: int, max_try: int):
    """
    Attempt pdb download from online database, either as .pdb- or .cif-file. 
    If this fails retry until max retry is reached
...
def accept_resolution_method_download(pdb: str, pdb_id: str,
                                      res_cutoff: float):
    """
    decide whether a pdb should be accepted based on the used method and 
    its resolution
